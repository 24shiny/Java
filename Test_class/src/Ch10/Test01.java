...
public class Test01 {
	public static void main(String[] args) {
		...
		Integer i1 = 123;
		arr[0] = s;
		arr[1] = i1; // different types
//		int len = arr[0].length();//Object에 length()가 없다
		int len = ((String) arr[0]).length();

		ArrayList lst = new ArrayList();
		//모든 타입의 객체들을 저장할 수 있다(Object객체로 간주) 그러나 비추다! - 이유는?
		//(num) num <- 10 (default)
		String s2 = "java";
		Integer i2 = 123; //boxed by Wrapper class
		lst.add(s2);
		lst.add(i2);
		...
		System.out.println(len_string); // the length of the first element : 2 서/울
